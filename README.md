...
当前分支是可本地运行的 `0.3.0-dev`。PydanticAI 单 Agent 只负责自然语言到严格 MissionDraft 的转换；确定性 Planner、Policy Engine 和独立 Verifier 负责时窗、候选、费用与报销判断。系统不会让模型编造车次、计算成本或执行购票订房。
...
当前本机结果（2026-07-31）：后端 `42 passed`；Alembic `upgrade head / check / downgrade 20260730_0002 / upgrade head` 通过；前端 `vue-tsc --noEmit && vite build` 成功；运行中 HTTP 冒烟与真实浏览器主链路通过。Docker CLI 未安装，因此容器构建未验证。详细记录见 [开发日志](docs/development-log.md)。
