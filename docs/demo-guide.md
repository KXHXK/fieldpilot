...
快速了解项目可先打开[线上静态专题](https://fieldpilot-kxh.netlify.app/)。专题页用于讲解业务问题、架构、检查点后缀重规划与验证边界；`0.6.0` 合并部署前，新增的政策版本、人工库存和风险过滤应使用本地工作台演示。
...
在首个现场任务点击“锁定至此”，再选择以下任一事件并点击“应用事件并重规划”：任务改期、总预算调整、未锁定交通取消/延误、受影响任务天气风险。最后回到首个现场任务点击“完成至此”。指出：
...
- 预算变化追加不可变政策快照并由 R2 绑定；交通与天气事件通过事件感知 Provider 过滤候选并保存过滤快照；
...
- 高德适配完成契约与故障测试，并提供脱敏真实验收工作流，但尚未使用个人 Key 完成生产复验；
- 铁路、航班和酒店支持冻结 Fixture 或严格授权人工导入，未抓取 12306，也不宣称自动同步实时库存；
- 天气和交通中断已进入确定性候选过滤，但事件仍由用户或上游明确提交，尚未订阅实时天气或运营中断流；
- 已实现受保护前缀不变的后缀重规划，但 Planner 仍是有界启发式搜索，不声称全局最优；
- Docker/PostgreSQL 配置已提供；Render 云端 Docker 已验证，本机仍无 Docker CLI 运行证据。
