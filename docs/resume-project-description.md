...
- 建立 Mission/PlanRevision/ReplanEvent/ExecutionCheckpoint/AgentRun 持久化，以输入指纹、命令幂等、乐观并发和严格前缀保护实现检查点后的后缀重规划；完成 51 项后端回归，并部署 Render Docker/FastAPI、Neon PostgreSQL 与 Netlify 工作台。
