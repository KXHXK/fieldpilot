...
- 铁路、航班、酒店可使用明确标记的 Fixture 或严格 Schema 的授权人工导入；人工数据强制标记 `manual` 并保存内容指纹，不抓取 12306 内部接口，也不冒充实时库存；
...
本地 `0.6.0` 已验证的是：55 项后端回归、Alembic `20260810_0005` 往返、不可变政策版本、授权人工库存导入、交通/天气候选过滤、工作台与专题页双构建；上一版还验证了 Neon、Render/Netlify 公网链路、R1/R2 smoke、重启后持久化、精确 CORS，以及上述 Kimi Live Eval。

尚未验证的是：真实高德 Key 的生产运行、自动同步的铁路/航班/酒店实时库存、生产限流、多租户身份隔离和 SLA；`0.6.0` 的公网迁移与部署也需在发布审批后复验。FieldPilot 展示的是可运行、可解释、可重规划的工程闭环，不声称已经具备真实下单能力或全局最优求解。
