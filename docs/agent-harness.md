...
`0.6.0` 已验证的是：55 项后端回归、Alembic `20260810_0005` 往返、不可变政策版本、授权人工库存导入、交通/天气候选过滤、工作台与专题页双构建；Neon、Render/Netlify 公网链路完成 R1～R5 smoke、政策版本查询、精确 CORS 与浏览器工作台复验，并保留上述 Kimi Live Eval。

尚未验证的是：真实高德 Key 的生产运行、自动同步的铁路/航班/酒店实时库存、生产限流、多租户身份隔离和 SLA。FieldPilot 展示的是可运行、可解释、可重规划的工程闭环，不声称已经具备真实下单能力或全局最优求解。
